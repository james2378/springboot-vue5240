...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"padding":"10px","margin":"10px 0 20px","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto","order":"2"}'>
      <el-form-item :style='{"margin":"5px 10px"}'>
		<div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>车次号</div>
        <el-select v-model="formSearch.checihao" placeholder="请选择车次号" :clearable="true">
          <el-option v-for="(item, index) in checihaoOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>上车点</div>
        <el-input v-model="formSearch.shangchedian" placeholder="上车点" clearable></el-input>
      </el-form-item>
      <el-form-item :style='{"margin":"5px 10px"}'>
	    <div class="lable" v-if="true" :style='{"width":"auto","padding":"0 10px","lineHeight":"42px","display":"inline-block"}'>下车点</div>
        <el-input v-model="formSearch.xiachedian" placeholder="下车点" clearable></el-input>
      </el-form-item>
	  ...
	  <el-button v-if="isAuth('bupiaoxinxi','新增')" :style='{"cursor":"pointer","border":"0","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#fff","borderRadius":"4px","background":"#00b9ea","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/bupiaoxinxiAdd')"><i v-if="true" :style='{"color":"#fff","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
...
<script>
  export default {
    ...
    created() {
      ...
      this.$http.get('option/huochexinxi/checihao').then(res => {
        if (res.data.code == 0) {
    ...
    methods: {
      ...
      getList(page, fenlei, ref = '') {
        ...
        let searchWhere = {};
        if (this.formSearch.checihao != '') searchWhere.checihao = this.formSearch.checihao;
        if (this.formSearch.shangchedian != '') searchWhere.shangchedian = '%' + this.formSearch.shangchedian + '%';
        if (this.formSearch.xiachedian != '') searchWhere.xiachedian = '%' + this.formSearch.xiachedian + '%';
	      params['sfsh'] = '是';
        this.$http.get('bupiaoxinxi/list', {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        this.$router.push({path: '/index/bupiaoxinxiDetail', query: {detailObj: JSON.stringify(item)}});
      },
    }
  }
</script>
